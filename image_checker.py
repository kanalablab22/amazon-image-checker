...
def _create_annotated_image(image: Image.Image, bbox: tuple) -> Image.Image:
    ...
    # 赤枠を商品から少し離す（マージン）
    bbox_margin = max(10, min(annotated.width, annotated.height) // 30)
    left, top, right, bottom = bbox
    left = max(0, left - bbox_margin)
    top = max(0, top - bbox_margin)
    right = min(annotated.width, right + bbox_margin)
    bottom = min(annotated.height, bottom + bbox_margin)
    bbox = (left, top, right, bottom)

    # 画像の周りに白い余白を追加
    padding = max(30, min(annotated.width, annotated.height) // 15)
...
def check_image(image: Image.Image, filename: str = "image.jpg", check_stitching: bool = False) -> ImageCheckReport:
    """
    画像の全チェックを実行して結果をまとめる
    check_stitching: True の場合、縫製チェックを実行する（革・布製品向け）
    """
    results = []
